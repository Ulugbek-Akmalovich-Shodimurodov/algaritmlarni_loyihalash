...
Btn.addEventListener("click", (e)=>{
    ...
    for (let i = 0; i < X_uzgaruvchilar.length; i++) {
        ...
        sum_X += Number(X_uzgaruvchilar[i]) - 0;
        sum_Y += Number(Y_uzgaruvchilar[i]) - 0;
        sum_XY += 1 * Number(X_uzgaruvchilar[i]) * Number(Y_uzgaruvchilar[i]);
        sum_XX += 1 * Number(X_uzgaruvchilar[i]) * Number(Y_uzgaruvchilar[i]);
        ssum_X += Number(X_uzgaruvchilar[i]) - 0;

        sum_X4 += Math.pow(Number(X_uzgaruvchilar[i]), 4);
        sum_X3 += Math.pow(Number(X_uzgaruvchilar[i]), 3);
        sum_X2 += Math.pow(Number(X_uzgaruvchilar[i]), 2);
        sum_xxy += Math.pow(Number(X_uzgaruvchilar[i]), 2)*Number(Y_uzgaruvchilar[i]);
    }
        ssum_X *= ssum_X;
        ...
        let detA = sum_X4*(sum_X2*X_uzgaruvchilar.length-sum_X*sum_X) - sum_X3*(sum_X3*X_uzgaruvchilar.length-sum_X*sum_X2)+sum_X2*(sum_X*sum_X3-sum_X2*sum_X2);
        let det_a = sum_xxy*(sum_X2*X_uzgaruvchilar.length-sum_X*sum_X)-sum_X3*(sum_XY * X_uzgaruvchilar.length - sum_X*sum_Y) + sum_X2*(sum_X*sum_XY-sum_Y*sum_X2);
        let det_b = sum_X4*(sum_XY*X_uzgaruvchilar.length-sum_Y*sum_X)-sum_xxy*(sum_X3*X_uzgaruvchilar.length-sum_X*sum_X2)+sum_X2*(sum_X3*sum_Y-sum_XY*sum_X2);
